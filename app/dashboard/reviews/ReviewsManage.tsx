...
export function ReviewsManage() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    if (!form.imageUrl.trim()) {
      setError(t(`${Rm}.validationImageRequired`));
      return;
    }
    setSaving(true);
    try {
      ...
      <section className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
        ...
        <form onSubmit={handleAdd} className="space-y-4">
          <div className="space-y-2">
            <label className="block text-sm font-medium text-[var(--color-foreground)]">{t(`${Rm}.labelReviewImageRequired`)}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--color-foreground)]">{t(`${Rm}.labelReviewTextOptional`)}</label>
            <textarea
              value={form.text}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="block text-sm font-medium text-[var(--color-foreground)]">{t(`${Rm}.labelAuthorNameOptional`)}</label>
              <input
                type="text"
                  ...
                  <>
                    {r.imageUrl ? (
                      <img
                        ...
                        alt={
                          r.authorName.trim()
                            ? fillMessage(t(`${Rm}.displayImageAlt`), { author: r.authorName.trim() })
                            : t(`${Rm}.imageAltPreview`)
                        }
                    ...
                    {r.text.trim() ? <p className="text-[var(--color-foreground)]">{r.text}</p> : null}
                    {r.authorName.trim() || r.authorTitle ? (
