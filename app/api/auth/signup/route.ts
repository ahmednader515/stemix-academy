...
import { getUserByEmail, getUserByWhatsappNumber, createUser } from "@/lib/db";
...
const signupSchema = z
  .object({
    ...
    whatsapp_number: z.string().min(1, "رقم الواتساب مطلوب"),
  })
  .refine(
  ...
  .refine(
    (data) => isValidEgyptWhatsapp(data.whatsapp_number),
    {
      message: "رقم الواتساب يجب أن يبدأ بـ 20 ويتكون من 12 رقماً (مثال: 201012345678)",
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, password, name, student_number, guardian_number, whatsapp_number } = parsed.data;
    ...
    const normalizedWhatsapp = normalizeEgyptWhatsapp(whatsapp_number);
    ...
    const whatsappTaken = await getUserByWhatsappNumber(normalizedWhatsapp);
    if (whatsappTaken) {
      return NextResponse.json(
        { error: "رقم الواتساب مستخدم لحساب آخر" },
        { status: 400 }
      );
    }
