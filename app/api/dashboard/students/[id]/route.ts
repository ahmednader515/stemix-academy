...
import { getUserById, getUserByEmailExcludingId, getUserByWhatsappNumber, updateUser, clearCurrentSessionId } from "@/lib/db";
...
export async function PATCH(
  ...
  if (body.whatsapp_number !== undefined) {
    ...
    if (!raw) {
    ...
    } else if (!isValidEgyptWhatsapp(raw)) {
      ...
      const taken = await getUserByWhatsappNumber(normalized, id);
      if (taken) {
        return NextResponse.json({ error: "رقم الواتساب مستخدم لحساب آخر" }, { status: 400 });
      }
