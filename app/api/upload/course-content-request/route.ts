...
/** رفع ملف لطلب شرح محتوى داخل دورة — للطالب فقط */
export async function POST(request: NextRequest) {
  ...
  if (!isR2Configured()) {
    ...
    return NextResponse.json({ error: "التخزين غير مضبوط. تواصل مع الإدارة." }, { status: 503 });
  }
  ...
  const key = `course-content-requests/${session.user.id}/${Date.now()}-${Math.random().toString(36).slice(2, 10)}.${safeExt}`;
  ...
  try {
    ...
    console.error("Course content request upload error:", e);
