...
/** رفع ملف مرجعي لطلب دورة (PDF أو صورة) — للطالب فقط */
export async function POST(request: NextRequest) {
  ...
  const key = `course-requests/${session.user.id}/${Date.now()}-${Math.random().toString(36).slice(2, 10)}.${safeExt}`;
  ...
  try {
    ...
    console.error("Course request upload error:", e);
