...
async function ensureWhatsappNumberColumn(): Promise<void> {
  return ensureOnce("ensureWhatsappNumberColumn", async () => {
    try {
      await sql`ALTER TABLE "User" ADD COLUMN IF NOT EXISTS whatsapp_number TEXT`;
    } catch {
...
export async function getUserByWhatsappNumber(
  ...
  await ensureWhatsappNumberColumn();
  const norm = whatsappNumber.replace(/\D/g, "");
  if (!norm) return null;
  const rows = excludeUserId
    ? await sql`
        SELECT * FROM "User"
        WHERE REGEXP_REPLACE(COALESCE(whatsapp_number, ''), '[^0-9]', '', 'g') = ${norm}
...
export async function createUser(data: {
  ...
  await ensureWhatsappNumberColumn().catch(() => {});
  const studentCopyright =
    (data.role ?? "STUDENT") === "STUDENT" ? await allocateUniqueCopyrightCode() : null;
  try {
    await sql`
      INSERT INTO "User" (id, email, password_hash, name, role, student_number, guardian_number, whatsapp_number, teacher_subject, teacher_avatar_url, copyright_code)
      VALUES (${id}, ${data.email}, ${data.password_hash}, ${data.name}, ${data.role ?? "STUDENT"}, ${data.student_number ?? null}, ${data.guardian_number ?? null}, ${data.whatsapp_number ?? null}, ${data.teacher_subject ?? null}, ${data.teacher_avatar_url ?? null}, ${studentCopyright})
    `;
  } catch (e) {
    ...
    if (missingTeacherCols) {
      ...
      await sql`
        INSERT INTO "User" (id, email, password_hash, name, role, student_number, guardian_number, whatsapp_number, copyright_code)
        VALUES (${id}, ${data.email}, ${data.password_hash}, ${data.name}, ${data.role ?? "STUDENT"}, ${data.student_number ?? null}, ${data.guardian_number ?? null}, ${data.whatsapp_number ?? null}, ${studentCopyright})
      `;
    } else {
      throw e;
    }
...
export async function updateUser(
  ...
  if (data.whatsapp_number !== undefined) await sql`UPDATE "User" SET whatsapp_number = ${data.whatsapp_number}, updated_at = NOW() WHERE id = ${id}`;
